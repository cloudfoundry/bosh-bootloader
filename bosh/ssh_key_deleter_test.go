...
var _ = Describe("SSHKeyDeleter", func() {
	Describe("Delete", func() {
		...
		Context("when the jumpbox-vars-store.yml exists on disk", func() {
			BeforeEach(func() {
				err := ioutil.WriteFile(filepath.Join(varsDir, "jumpbox-vars-store.yml"), []byte(beforeDeletionVars), storage.StateMode)
				Expect(err).NotTo(HaveOccurred())
			})
			It("deletes the jumpbox key from the file on disk", func() {
				...
				contents, err := ioutil.ReadFile(filepath.Join(varsDir, "jumpbox-vars-store.yml"))
				Expect(err).NotTo(HaveOccurred())
		...
		Context("when the jumpbox-vars-store.yml doesn't contain ssh key", func() {
			BeforeEach(func() {
				err := ioutil.WriteFile(filepath.Join(varsDir, "jumpbox-vars-store.yml"), []byte(afterDeletionVars), storage.StateMode)
				Expect(err).NotTo(HaveOccurred())
			})
		...
		Context("when the jumpbox-vars-store.yml does not exist on disk", func() {
			It("does not write the file to disk", func() {
				...
				_, err = os.Stat(filepath.Join(varsDir, "jumpbox-vars-store.yml"))
		...
		Context("when the Jumpbox variables is invalid YAML", func() {
			BeforeEach(func() {
				err := ioutil.WriteFile(filepath.Join(varsDir, "jumpbox-vars-store.yml"), []byte("invalid yaml"), storage.StateMode)
				Expect(err).NotTo(HaveOccurred())
			})
