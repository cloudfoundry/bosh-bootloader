...
output "external_ip" {
  value = "${google_compute_address.jumpbox-external-ip.address}"
}
...
output "bosh_open_tag_name" {
  value = "${google_compute_firewall.jumpbox-open.name}"
}
...
output "director_address" {
  value = "https://${google_compute_address.jumpbox-external-ip.address}:25555"
}
...
resource "google_compute_address" "jumpbox-external-ip" {
  name = "${var.env_id}-jumpbox-external-ip"
}

resource "google_compute_firewall" "jumpbox-open" {
  name    = "${var.env_id}-jumpbox-open"
  ...
  target_tags = ["${var.env_id}-jumpbox-open"]
}
