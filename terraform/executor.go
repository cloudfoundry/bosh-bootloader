...
func (e Executor) Apply(credentials, envID, projectID, zone, region, cert, key, systemDomain, template, prevTFState string) (string, error) {
	tempDir, err := tempDir("", "")
	if err != nil {
		return "", err
	}
	...
	args = append(args, makeVar("system_domain", systemDomain)...)
