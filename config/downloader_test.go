...
var _ = Describe("BBL State Downloader", func() {
	...
	Describe("UntarStateDir", func() {
		...
		BeforeEach(func() {
			var err error
			stateReader, err = os.Open("fixtures/some-tar.tgz")
			Expect(err).NotTo(HaveOccurred())
		})
		AfterEach(func() {
			os.RemoveAll(stateDirPath)
		})
		It("untars the bbl state and places it in temp dir", func() {
			stateDirPath, err := downloader.UntarStateDir(stateReader)
			Expect(err).NotTo(HaveOccurred())
			Expect(filepath.Join(stateDirPath, "some-file")).To(BeAnExistingFile())
		})

		Context("when an invalid stateTar is passed", func() {
			It("errors", func() {
				stateReader = strings.NewReader("")
				_, err := downloader.UntarStateDir(stateReader)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("unable to untar state dir"))
			})
		})
	})
})
