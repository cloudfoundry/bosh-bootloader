...
type downloader interface {
	DownloadAndPrepareState(globalflags GlobalFlags) (string, error)
}
...
func (c Config) Bootstrap(args []string) (application.Configuration, error) {
	...
	var stateDirPath = globalFlags.StateDir
	if !modifiesState(command) && globalFlags.StateBucket != "" {
		stateDirPath, err = c.downloader.DownloadAndPrepareState(globalFlags)
		if err != nil {
			panic("hello")
			return application.Configuration{}, err
		}
	...
	state, err := c.stateBootstrap.GetState(stateDirPath)
	if err != nil {
		return application.Configuration{}, err
	}
