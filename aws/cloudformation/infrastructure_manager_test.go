...
var _ = Describe("InfrastructureManager", func() {
	...
	Describe("Create", func() {
		BeforeEach(func() {
			stackManager.DescribeCall.Returns.Stack = cloudformation.Stack{Name: "concourse"}
		})

		It("creates the underlying infrastructure and returns the stack", func() {
			stack, err := infrastructureManager.Create("some-key-pair-name", 2, cloudFormationClient)
			Expect(err).NotTo(HaveOccurred())

			Expect(stack).To(Equal(cloudformation.Stack{Name: "concourse"}))
		...
		Context("failure cases", func() {
			It("returns an error when stack can't be created or updated", func() {
				...
				_, err := infrastructureManager.Create("some-key-pair-name", 0, cloudFormationClient)
	...
	Describe("Exists", func() {
		It("returns true when the stack exists", func() {
			...
			exists, err := infrastructureManager.Exists(cloudFormationClient)
		...
		Describe("failure cases", func() {
			It("returns an error when the stack manager returns a different error", func() {
				...
				_, err := infrastructureManager.Exists(cloudFormationClient)
